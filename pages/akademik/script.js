...
document.getElementById('jadwalForm').addEventListener('submit', async function(e) {
  ...
  const existing = await fetch(`http://localhost:3001/list-jadwal?userId=${data.userId}`);
...
    border:true
  ...
  try {
    const res = await fetch('http://localhost:3001/input-jadwal', {
      method: 'POST',
...
function toggleEditRow(button, id) {
  ...
  if (sedangEdit) {
    ...
    fetch('http://localhost:3001/update-jadwal', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
async function hapusJadwal(id) {
  ...
  try {
    const res = await fetch(`http://localhost:3001/hapus-jadwal/${id}`, {
      method: "DELETE"
    });
...
async function tampilkanQuizHariIni() {
  ...
  const userId = localStorage.getItem("userId");
  const res = await fetch(`http://localhost:3001/jadwal-hari-ini?userId=${userId}`);
